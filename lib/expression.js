...
CronExpression._parseField = function _parseField (field, value, constraints) {
  ...
  function parseRange (val, repeatInterval) {
    ...
    if (atoms.length === 1 && repeatInterval > 1) {
      throw new Error('Invalid range: ' + val + "/" + repeatInterval);
    }
...
CronExpression.prototype._findSchedule = function _findSchedule (reverse) {
  ...
  while (true) {
    ...
    //
    //
    // ** However, in order to make it behave the same as Spring Framework, in this fork, we use AND instead of OR **
    //
    //
    ...
    if (!isMonthWildcardMatch) {
      ...
      if (this._fields.month[0] === previousMonth && _daysInPreviousMonth < _daysOfMontRangeMax && !isDayOfMonthWildcardMatch) {
    ...
    // Add or subtract day if day of month and week are non-wildcard values and either value doesn't match
